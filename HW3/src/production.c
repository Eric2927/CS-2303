...
bool production(int argc, char* argv[])
{
	...
	if(argc <=1) //no interesting information
	{
		...
		puts("Please provide a room limit and a treasure limit.");
	}
	else if (argc == 3) {
		puts("Please provide a treasure limit.");
	}
				...
				for(int col = 0; (col<nrooms)&&!done; col++)
				{
					...
					if(getEdge(adjMP,roomBeingSearchedP->roomNumber, col)==1)
					{
						...
						{//if it hasn't been searched
							...
							if(searchedRooms < maxRooms && foundTreasure + theRoomPs[col]->treasure < maxTreasure) //TODO:what's the condition?
								//we check whether we can take the treasure vs. limit
								//we check whether we've hit the room limit
							{//we enqueue it for search
								foundTreasure += theRoomPs[col]->treasure;//TODO what goes here?
								...
								savePayload(searchQ, theRoomPs[col]);//TODO: what goes here?
